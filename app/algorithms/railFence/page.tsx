...
import { railFenceEncrypt, railFenceDecrypt } from './rail_fence';
...
const RailFence = () => {
  ...
  const processMessage = (message: string, key: number, isEncrypt: boolean) =>
    isEncrypt ? railFenceEncrypt(message, key) : railFenceDecrypt(message, key);
  ...
  const handleSendMessage = (sender: 'user1' | 'user2', message: string, key: string, isEncrypt: boolean) => {
    ...
    setMessages((prev) => [
      ...
      {
        sender,
        text: processMessage(message, parseInt(key, 10), isEncrypt),
        key: parseInt(key, 10),
